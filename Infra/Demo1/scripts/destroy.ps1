...
if ($NoWait) {
    Write-Host "Delete started in background. Check status with:"
    Write-Host "  az group show --name $ResourceGroup --query properties.provisioningState -o tsv"
}
else {
    Write-Host "Done. Resource group '$ResourceGroup' and all contained resources have been deleted."
}
